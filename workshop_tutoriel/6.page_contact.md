...
## View
On va d'abord créer une view pour le formulaire de contact
...
## Classe Mail
Pour envoyer des mails dans Laravel, il faudra créer une classe mail en faisant
```bash 
php artisan make:mail Contact
```
Dans la classe Mail, il faudra définir les variables (sous forme de propriétés publiques) qu'on voudrait passer dans la view contact.
...
Dans le controller, on va créer deux fonctions. On va d'abord créer une fonction pour afficher le formulaire avec les champs préremplis.
```php
...
```
Ensuite, on va créer un autre fonction pour gérer le request et puis passer les informations dans la classe Mail\Contact
...
Enfin, on va définir nos routes pour que l'application fonctionne.
